...
# --- LOGIQUE DE CALCUL DOUANIER (BACKEND) ---
...
@app.route('/')
def index():
    # Rendu du frontend directement depuis la chaîne HTML ci-dessous
...
def calculate():
    ...
    # Calcul de l'Assurance selon le mode sélectionné
    ass_total = 0.0
    mode_ass = data.get('mode_assurance')
    fob_total = float(data.get('fob_total', 0))
    cfr_global = fob_total + fret_global
    ...
    if choix == 1:
        caf = fob_total + fret_global + ass_total
        ...
        for i in range(2):
            ...
            ass_art = ass_total * (fob_art / fob_total) if fob_total > 0 else 0.0
...
# --- INTERFACE GRAPHIQUE (FRONTEND HTML/CSS/JS) ---
HTML_TEMPLATE = """
<!DOCTYPE html>
<html lang="fr">
<head>
    ...
    <title>Calculateur Douanier Professionnel</title>
    <script src="https://cdn.tailwindcss.com"></script>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&display=swap" rel="stylesheet">
    <style>
        body { font-family: 'Inter', sans-serif; background-color: #f8fafc; }
        .card { background: white; border-radius: 12px; box-shadow: 0 4px 6px -1px rgb(0 0 0 / 0.05), 0 2px 4px -2px rgb(0 0 0 / 0.05); border: 1px solid #e2e8f0; }
    </style>
</head>
<body class="p-4 md:p-8">
    ...
    <div class="max-w-6xl mx-auto">
        <header class="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 pb-6 border-b border-slate-200 gap-4">
            <div>
                ...
                <p class="text-slate-500 mt-1">Interface de liquidation douanière automatisée (Normes UEMOA)</p>
            </div>
            <span class="bg-blue-50 text-blue-700 px-4 py-1.5 rounded-full text-sm font-semibold border border-blue-100">Système Expert v2.0</span>
        </header>
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="card p-6">
                    ...
                    <div class="flex gap-4">
                        ...
                        <button onclick="setChoix(2)" id="btn-2" class="flex-1 py-3 rounded-xl border-2 font-bold transition-all">2 Articles Distinctes</button>
                    </div>
                </div>
    ...
    <script>
                ...
                <div class="mb-6 p-4 bg-slate-50 rounded-xl border border-slate-200">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <input type="number" id="fob_u_${i}" value="0" class="w-full mt-1 border border-slate-300 rounded-lg p-2 bg-white">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold uppercase text-slate-500">Nombre de Colis</label>
                            <input type="number" id="qty_${i}" value="0" class="w-full mt-1 border border-slate-300 rounded-lg p-2 bg-white">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold uppercase text-slate-500">Poids Brut (kg)</label>
                            <input type="number" id="poids_${i}" value="0" step="0.1" class="w-full mt-1 border border-slate-300 rounded-lg p-2 bg-white" ${choixActuel === 1 ? 'disabled placeholder="Non requis"' : ''}>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4 pt-3 border-t border-slate-200/60">
                        <div>
                            ...
                            <select id="cat_${i}" class="w-full mt-1 border border-slate-300 rounded-lg p-2 bg-white">
                        ...
                        <div class="flex flex-col justify-end space-y-2 pb-1">
                            ...
                            <div id="assise_val_container_${i}" class="hidden">
                                <input type="number" id="assise_taux_${i}" value="5" class="w-24 border border-slate-300 rounded-lg p-1 text-sm inline-block"> % taux d'assise
                            </div>
                            <label class="flex items-center gap-2 text-sm text-slate-700 cursor-pointer">
        ...
        function toggleAssimerInput(i) { /* Legacy check fix */ }
        ...
        function displayResults(data) {
            ...
            let html = `
                <div class="bg-blue-600 text-white text-center p-4 rounded-xl shadow-inner">
                    <span class="text-xs uppercase tracking-wider opacity-80">Montant Global Combined à Payer</span>
            ...
            data.results.forEach((res, index) => {
                html += `
                    <div class="bg-white p-4 rounded-xl border border-slate-200">
                        <div class="flex justify-between items-center mb-3 pb-2 border-b border-slate-100">
                            <h4 class="font-bold text-slate-800">${choixActuel === 1 ? 'Bilan unique' : 'Article ' + (index + 1)}</h4>
                ...
                res.elements.forEach(el => {
                    const isTotalField = el.label.includes('CAF');
                    html += `
                        <div class="flex justify-between ${isTotalField ? 'font-semibold text-slate-900 pt-1' : 'text-slate-500'}">
                            <span>${el.label}</span>
                            <span>${el.val.toLocaleString('fr-FR')}</span>
                        </div>
                    `;
                });
